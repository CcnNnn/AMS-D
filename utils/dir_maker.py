# Copyright (c) 2021-present LG CNS Corp.
# original code from:  
# "Self-Knowledge Distillation with Progressive Refinement of Targets"
# Kyungyul Kim, ByeongMoon Ji, Doyoung Yoon, Sangheum Hwang, ICCV 2021
# GitHub: https://github.com/lgcnsai/PS-KD-Pytorch


import os
from datetime import datetime

#--------------
#util
#--------------
from .color import Colorer

class DirectroyMaker:
    C = Colorer.instance()
    def __init__(self, root, save_model=True, save_log=True, save_config=True):
        self.root = root
        self.save_model  = save_model
        self.save_log = save_log
        self.save_config = save_config
        
    
    def experiments_dir_maker(self,args):
        
        if not os.path.exists(self.root):
            os.makedirs(self.root)
        
        now = datetime.now()
        time_idx = '_%s-%s-%s-%s-%s-%s' % (now.year, now.month, now.day, now.hour, now.minute, now.second)
        # --save_dir
        detail_dir = args.data_type + "_" +  "_PSKD_" + time_idx
        
        create_dir_list = []
        if self.save_model:
            new_path = os.path.join(self.root, detail_dir, 'model')
            create_dir_list.append(new_path)
        if self.save_log:
            new_path = os.path.join(self.root, detail_dir, 'log')
            create_dir_list.append(new_path)    
        if self.save_config:
            new_path = os.path.join(self.root, detail_dir, 'config')
            create_dir_list.append(new_path)
        
        for path in create_dir_list:
            if not os.path.exists(path):
                os.makedirs(path)
                
        print(self.C.violet2("[Directory] save_dir: {}".format(create_dir_list[0])))
        print(self.C.violet2("[Directory] log_dir: {}".format(create_dir_list[1])))
        print(self.C.violet2("[Directory] config_dir: {}".format(create_dir_list[2])))
        
        return create_dir_list